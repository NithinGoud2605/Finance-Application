...
import { Box, Typography, Paper, Grid, Stack, Table, TableBody, TableCell, TableHead, TableRow, Divider } from '@mui/material';
...
const InvoiceTemplate4 = ({ sender = {}, receiver = {}, details = {}, charges = {}, paymentInformation = {}, logo, forPdf = false }) => {
  // Styling constants
  const effectiveBg = '#ffffff';
  const effectiveTextPrimary = '#1a237e';
  const effectiveTextSecondary = '#3949ab';
  const effectiveAccent = '#e8eaf6';
  ...
  // Logo handling with proper default
          ...
          <motion.div variants={itemVariants}>
            <Box sx={{ display: 'flex', justifyContent: 'space-between', mb: 3 }}>
              ...
              <Box sx={{ textAlign: 'right' }}>
                {details.issueDate && (
                  <Typography sx={{ color: effectiveTextSecondary, mb: 1, ...noHyphenStyle }}>
                    Issue Date: {formatDate(details.issueDate)}
                  </Typography>
                )}
                {details.dueDate && (
                  <Typography sx={{ color: effectiveTextSecondary, ...noHyphenStyle }}>
                ...
                <TableBody>
                  {items.map((item, index) => (
                    <TableRow 
                      key={index}
                      sx={{ 
                        '&:nth-of-type(odd)': { bgcolor: effectiveAccent },
